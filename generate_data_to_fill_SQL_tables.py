...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar  1 22:04:58 2018
...
def randomDate():
    date = random_date_generator('2017-01-01', 450)
    return str(date) 
...
# save data as csv
def save_result_as_csv(name,a,b,c,d,f):
    filename = "%s_%s.%s" % (name, getToday() ,"csv")
    with open(filename, 'w') as outcsv:
        writer = csv.writer(outcsv)
#        writer.writerow(["Client Name & ID", "State", "Date", "Income", "Expense", "Tax"])
        for row in zip_longest(a,b,c,d,f): # writing to csv with longest list index
            writer.writerow(row)
...
save_result_as_csv('food_truck_1',clnt, st, dte, incm, tx)
...
save_result_as_csv('bread_factory_2',clntBF, stBF, dteBF, incmBF, txBF)
...
save_result_as_csv('sandwich_shop_3',clntSS, stSS, dteSS, incmSS, txSS)
...
save_result_as_csv('lagguge_company_4',clntLC, stLC, dteLC, incmLC, txLC)
...
save_result_as_csv('battery_company_5',clntBC, stBC, dteBC, incmBC, txBC)
...
#print(len(allDate))
save_result_as_csv('all_data',allClient,allLocation,allDate,allIncome,allTaxAmmount)
